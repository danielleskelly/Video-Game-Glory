...
func _process(delta):
	if (old_parent.get_child_count() > 0) and (is_in_group("free") == true): #if there is a customer waiting in line and arcade zone one is available
...
func set_genre_type(): #sets the genre of the games based on the town
	if global.town_select == "hollyhock":
		if (global.station_one_selection == 0):
			genre_type = "none"
		if (global.station_one_selection == 1):
			genre_type = "meta"
			prediction_one = global.meta_prediction
		if (global.station_one_selection == 2):
			genre_type = "classic"
			prediction_two = global.classic_prediction
		if (global.station_one_selection == 3):
