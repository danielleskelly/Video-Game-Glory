extends Node2D #deals with arcade zone four and moving the customer through it
...
onready var new_parent = get_tree().get_current_scene().get_node("zone_four_path")
var move_node
...
func _process(delta):
	...
	#if there is a customer waiting in line and arcade zone four is available
	if (old_parent.get_child_count() > 0):
		if (is_in_group("free") == true):
			...
			if (global.town_select == "hollyhock"):
				...
				if (move_node.get_child(0).is_in_group("meta") == true) and (set_genre.genre_type_four == "meta") and (move_node.get_unit_offset() > 1): 
					move_along_first_in_line()
				#same story different genre
				if (move_node.get_child(0).is_in_group("classic") == true) and (set_genre.genre_type_four == "classic") and (move_node.get_unit_offset() > 1): #check to see if the customer wants what the arcade is set to
					move_along_first_in_line()
				#same story different genre
				if (move_node.get_child(0).is_in_group("platformer") == true) and (set_genre.genre_type_four == "platformer") and (move_node.get_unit_offset() > 1): #check to see if the customer wants what the arcade is set to
					move_along_first_in_line()
