...
func _process(delta):
	...
	if (towns.town_select == "plansey"):
		...
		if (plansey.plansey_arcade_best_key == false):
			station_six.set_hidden(true)
		set_genre.plansey_station_one_selection = get_node("station_one/dropdown").get_selected()
		set_genre.plansey_station_two_selection = get_node("station_two/dropdown").get_selected()
		set_genre.plansey_station_three_selection = get_node("station_three/dropdown").get_selected()
		set_genre.plansey_station_four_selection = get_node("station_four/dropdown").get_selected()
		set_genre.plansey_station_five_selection = get_node("station_five/dropdown").get_selected()
		set_genre.plansey_station_six_selection = get_node("station_six/dropdown").get_selected()
...
func set_games():
	...
	if (towns.town_select == "plansey"):
		station_one.get_child(1).select(set_genre.plansey_station_one_selection)
		station_two.get_child(1).select(set_genre.plansey_station_two_selection)
		if (keys.plansey_station_three_key == true):
			station_three.get_child(1).select(set_genre.plansey_station_three_selection)
		if (keys.plansey_station_four_key == true):
			station_four.get_child(1).select(set_genre.plansey_station_four_selection)
		if (keys.plansey_station_five_key == true):
			station_five.get_child(1).select(set_genre.plansey_station_five_selection)
		if (keys.plansey_station_six_key == true):
			station_six.get_child(1).select(set_genre.plansey_station_six_selection)
