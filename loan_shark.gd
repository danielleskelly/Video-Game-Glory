...
func _on_payback_raise_button_down():
	...
	if (towns.town_select == "untilly"):
		if ((int(payback) <= untilly.untilly_current_loan) and (money.untilly_cash >= payback)):
			...
			payback_output.set_text(str(payback))
	if (towns.town_select == "windrow"):
		if ((int(payback) <= windrow.windrow_current_loan) and (money.windrow_cash >= payback)):
			payback = int(payback) + 1
			payback_output.clear()
			payback_output.set_text(str(payback))
	if (towns.town_select == "banlon"):
		if ((int(payback) <= banlon.banlon_current_loan) and (money.banlon_cash >= payback)):
			payback = int(payback) + 1
			payback_output.clear()
			payback_output.set_text(str(payback))
	if (towns.town_select == "slatten"):
		if ((int(payback) <= slatten.slatten_current_loan) and (money.slatten_cash >= payback)):
			payback = int(payback) + 1
			payback_output.clear()
			payback_output.set_text(str(payback))
	if (towns.town_select == "plansey"):
...
func _on_confirm_button_down():
	if (towns.town_select == "hollyhock"):
		...
		if (int(borrow) > (1000 - hollyhock.hollyhock_current_loan)):
			borrow = 1000 - hollyhock.hollyhock_current_loan
		...
		loans.credit_limit = 1000 - hollyhock.hollyhock_current_loan
	if (towns.town_select == "fiyork"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - fiyork.fiyork_current_loan)):
			borrow = 1000 - fiyork.fiyork_current_loan
		...
		loans.credit_limit = 1000 - fiyork.fiyork_current_loan
	if (towns.town_select == "plansey"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - plansey.plansey_current_loan)):
			borrow = 1000 - plansey.plansey_current_loan
		...
		loans.credit_limit = 1000 - plansey.plansey_current_loan
	if (towns.town_select == "untilly"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - untilly.untilly_current_loan)):
			borrow = 1000 - untilly.untilly_current_loan
		...
		loans.credit_limit = 1000 - untilly.untilly_current_loan
	if (towns.town_select == "windrow"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - windrow.windrow_current_loan)):
			borrow = 1000 - windrow.windrow_current_loan
		...
		loans.credit_limit = 1000 - windrow.windrow_current_loan
	if (towns.town_select == "banlon"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - banlon.banlon_current_loan)):
			borrow = 1000 - banlon.banlon_current_loan
		...
		loans.credit_limit = 1000 - banlon.banlon_current_loan
	if (towns.town_select == "slatten"):
		borrow = get_node("borrow_output").get_text()
		payback = get_node("payback_output").get_text()
		if (int(borrow) > (1000 - slatten.slatten_current_loan)):
			borrow = 1000 - slatten.slatten_current_loan
		...
		loans.credit_limit = 1000 - slatten.slatten_current_loan
	borrow = 0
	payback = 0
	borrow_output.clear()
	borrow_output.set_text(str(borrow))
