	...
	if line_ten_clear == true:
		...
		if nine.empty() == false:
			for x in nine:
				...
				x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("1_10").get_global_position().y))
				x.remove_from_group("nine")
				x.add_to_group("ten")
		var eight = get_tree().get_nodes_in_group("eight")
		for x in eight:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("19").get_global_position().y))
			x.remove_from_group("eight")
			x.add_to_group("nine")
		var seven = get_tree().get_nodes_in_group("seven")
		for x in seven:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("18").get_global_position().y))
			x.remove_from_group("seven")
			x.add_to_group("eight")
		var six = get_tree().get_nodes_in_group("six")
		for x in six:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("17").get_global_position().y))
			x.remove_from_group("six")
			x.add_to_group("seven")
		var five = get_tree().get_nodes_in_group("five")
		for x in five:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("16").get_global_position().y))
			x.remove_from_group("five")
			x.add_to_group("six")
		var four = get_tree().get_nodes_in_group("four")
		for x in four:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("15").get_global_position().y))
			x.remove_from_group("four")
			x.add_to_group("five")
		var three = get_tree().get_nodes_in_group("three")
		for x in three:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("14").get_global_position().y))
			x.remove_from_group("three")
			x.add_to_group("four")
		var two = get_tree().get_nodes_in_group("two")
		for x in two:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("13").get_global_position().y))
			x.remove_from_group("two")
			x.add_to_group("three")
		var one = get_tree().get_nodes_in_group("one")
		for x in one:
			var pos = x.get_global_position()
			x.set_global_position(Vector2(pos.x, x.get_parent().get_parent().get_node("12").get_global_position().y))
