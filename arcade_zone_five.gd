...
func _process(delta):
	if (old_parent.get_child_count() > 0) and (is_in_group("free") == true): #if there is a customer waiting in line and arcade zone five is available
		...
		if (move_node.get_child(0).is_in_group("meta") == true) and (genre_type == "meta") and (move_node.get_unit_offset() > 1): #check to see if the customer wants what the arcade is set to
			var check = global.arcade_five_range_high - global.arcade_five_price #check if the arcade price is too high
...
func set_genre_type(): #sets the genre of the games based on the town
	if global.town_select == "hollyhock":
		if (global.station_five_selection == 0):
			genre_type = "none"
		if (global.station_five_selection == 1):
			genre_type = "meta"
			prediction_one = global.meta_prediction
		if (global.station_five_selection == 2):
			genre_type = "classic"
			prediction_two = global.classic_prediction
		if (global.station_five_selection == 3):
