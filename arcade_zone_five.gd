...
func set_type():
	if town_select == "hollyhock":
		if (station_five_selection == 0):
			type = "none"
		if (station_five_selection == 1):
			type = "meta"
		if (station_five_selection == 2):
			type = "classic"
		if station_five_selection == 3:
			type = "platformer"
	
	
func get_globals():
	station_five_selection = global.station_five_selection
